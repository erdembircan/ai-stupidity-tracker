...
- The [formula page](https://erdembircan.github.io/ai-stupidity-tracker/) now carries a "What's new" section, published from this changelog on every release — so the page says what shipped instead of only which version did

## [1.3.2] - 2026-08-16

### Added

- `EXCELLENT` model status support — the API's top grade now renders with a filled gold star (`★`) instead of falling through to the generic failure cross. The grade is assigned by the API and is independent of a model's position on the leaderboard

### Fixed

- Models graded `excellent` are now included in the `Global AI Index` performing-well list, which previously matched only `good` and silently dropped the highest-rated models in the index
...
[1.7.0]: https://github.com/erdembircan/ai-stupidity-tracker/compare/v1.6.0...v1.7.0
[1.6.0]: https://github.com/erdembircan/ai-stupidity-tracker/compare/v1.5.1...v1.6.0
